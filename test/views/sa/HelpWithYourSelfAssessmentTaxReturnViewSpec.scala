...
class HelpWithYourSelfAssessmentTaxReturnViewSpec extends ViewBehaviours {
  ...
  def createView =
    () =>
      help_with_your_self_assessment_tax_return(frontendAppConfig)(
  ...
  "SelfAssessmentTaxReturnCheck view" must {
    ...
    "contain heading ID" in {
      ...
      doc
        .getElementsByTag("h1")
        .attr("id") mustBe "help-with-your-sa-tax-return"
    }
    ...
    "have correct content" in {
      ...
      doc.text() must include(
        "If you are sent a tax return, or if you get an email or letter from HMRC telling you to complete one, " +
      ...
      doc.text() must include(
        "Tax is usually deducted automatically from wages, pensions and savings. People and businesses with " +
          "other income must report it in a tax return.")

      doc.text() must include(
        "Self Assessment is not just for self-employed people. Everyone who has to complete a tax return must " +
      ...
      doc.text() must include(
        "If you did not send an online return last year, allow extra time (up to 20 working days) as you will " +
          "need to register first.")

      doc.text() must include(
        "You will tailor your tax return on the first 3 pages. Use this section to tell us about your different " +
          "types of income, employments and self-employments so that you only see the parts of the tax return that you need to.")

      doc.text() must include(
        "If you started or ceased self-employment during the year, it is important to enter the dates so HMRC " +
          "can update your records.")

      doc.text() must include(
        "If you did not trade for a full year, but if you had and your income would have been over £85,000, " +
          "you answer “yes”. For example: If your business income over 6 months was £50,000, then for 12 months it would have been " +
          "£100,000.")

      doc.text() must include(
        "Do not fill in this section if you run a guest house or offer bed and breakfast. That is classed as " +
          "self-employment and you should fill in that section.")

      doc.text() must include(
        "If your property is outside the European Economic Area, fill in the Foreign section.")

      doc.text() must include(
        "If your property income was from a partnership, fill in the Partnership section.")
    }
    ...
    "have youtube url in html for each embedded video" in {
      val doc = asDocument(createView())
      val listOfVideoId: List[String] = List("5qOq9nWx-0c",
                                             "D-WSq_vSTU8",
                                             "8I9A4ZefLPU",
                                             "htUNA2b7Gbk",
                                             "ZKKVd1XQQJA")
