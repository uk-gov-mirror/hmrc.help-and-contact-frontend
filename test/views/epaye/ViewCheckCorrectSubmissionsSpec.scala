...
class ViewCheckCorrectSubmissionsSpec extends ViewBehaviours{
  ...
  def createView = () => inject[view_check_correct_submissions].apply(frontendAppConfig, Some("user@example.com"), false)(Some(HtmlFormat.empty))(fakeRequest, messages)
  def createViewWithoutEmail = () => inject[view_check_correct_submissions].apply(frontendAppConfig, None, false)(Some(HtmlFormat.empty))(fakeRequest, messages)
