...
class HelpAndContactController @Inject()(appConfig: FrontendAppConfig,
  ...
  def onPageLoad(category: HelpCategory, page: String) =
  ...
  private def corporationTax(page: String)(
      implicit request: ServiceInfoRequest[AnyContent]) = {
    page match {
      case "contact-hmrc" =>
        Ok(contact_hmrc_about_ct(appConfig)(request.serviceInfoContent))
      case "how-to-pay" =>
        Ok(how_to_pay_corporation_tax(appConfig)(request.serviceInfoContent))
      case "register-or-tell-hmrc-you-are-no-longer-trading" =>
        Ok(
          register_or_deregister_corporation_tax(appConfig)(
            request.serviceInfoContent))
      case _ => NotFound(errorHandler.notFoundTemplate)
    }
  }

  private def ePaye(page: String)(
      implicit request: ServiceInfoRequest[AnyContent]) = {
    page match {
      case "get-started" =>
        MovedPermanently("/business-account/epaye/get-started")
      case "remove" => MovedPermanently("/business-account/epaye/remove")
      case "refunds" =>
        Ok(paye_and_cis_refunds(appConfig)(request.serviceInfoContent))
      case "view-check-correct-submissions" =>
        Ok(
          view_check_correct_submissions(appConfig, request.request.email)(
            request.serviceInfoContent))
      case "change-employee-circumstances" =>
        MovedPermanently(
      ...
      case "latency" =>
        MovedPermanently(
          "/business-account/help/epaye/view-check-correct-submissions")
      case "paye-refund" =>
        MovedPermanently("/business-account/help/epaye/refunds")
      case "" => MovedPermanently("/business-account/help")
      case _  => NotFound(errorHandler.notFoundTemplate)
    }
  }

  private def selfAssessment(page: String)(
      implicit request: ServiceInfoRequest[AnyContent]) = {
    page match {
      ...
      case "help-with-return" =>
        Ok(
          help_with_your_self_assessment_tax_return(appConfig)(
            request.serviceInfoContent))
      case "how-to-pay" =>
        Ok(how_to_pay_self_assessment(appConfig)(request.serviceInfoContent))
      case "register-or-deregister" =>
        Ok(register_deregister(appConfig)(request.serviceInfoContent))
      case _ => NotFound(errorHandler.notFoundTemplate)
    }
  }

  private def vat(page: String)(
      implicit request: ServiceInfoRequest[AnyContent]) = {
    page match {
      case "how-to-pay" =>
        Ok(payments_and_deadlines(appConfig)(request.serviceInfoContent))
      case "register-or-deregister" =>
        Ok(register_or_deregister(appConfig)(request.serviceInfoContent))
      case _ => NotFound(errorHandler.notFoundTemplate)
    }
  }
