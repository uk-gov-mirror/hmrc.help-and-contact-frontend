...
class ViewCheckCorrectSubmissionsSpec extends ViewBehaviours {
  ...
  def createView =
    () =>
      view_check_correct_submissions(
        frontendAppConfig,
        Some("user@example.com"))(HtmlFormat.empty)(fakeRequest, messages)
  def createViewWithoutEmail =
    () =>
      view_check_correct_submissions(frontendAppConfig, None)(HtmlFormat.empty)(
        fakeRequest,
        messages)
  ...
  "View Check Correct Submissions view with an email accessible" must {
    ...
    "contain heading ID" in {
      ...
      doc
        .getElementsByTag("h1")
        .attr("id") mustBe "view-check-correct-submission"
    }

    "contain correct dynamic content" in {
      doc.text() must include(
        "We send an email to your Government Gateway address each time we receive a Full Payment Submission (FPS) or Employer Payment Summary (EPS).")
      doc.text() must include(
        "Your Government Gateway email is user@example.com")
    }
  ...
  "View Check Correct Submissions view with no email accessible" must {
    ...
    "contain correct dynamic content" in {
      docWithoutEmail.text() must include(
        "We can send you an email each time we receive a Full Payment Submission (FPS) or Employer Payment Summary (EPS), if you")
