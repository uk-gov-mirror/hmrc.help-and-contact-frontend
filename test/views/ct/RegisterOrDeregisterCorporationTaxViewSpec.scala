...
class RegisterOrDeregisterCorporationTaxViewSpec extends ViewBehaviours {
  ...
  "/help/corporation-tax/register-or-tell-hmrc-you-are-no-longer-trading" must {
    ...
    "have correct content" in {
      val doc = asDocument(createView())
      doc.text() must include("After you have registered your company with Companies House, you will need to register it for Corporation Tax within 3 months of starting to do business.")

      doc.text() must include("This includes buying, selling, advertising, renting a property and employing someone. You can check if you’re unsure what counts as starting to do business.")

      doc.text() must include("You may get a penalty if you register late.")

      doc.text() must include("You’ll need your company’s 10-digit Unique Taxpayer Reference (UTR) before you register for Corporation Tax online.")

      doc.text() must include("This is posted to your company address by HM Revenue and Customs (HMRC), usually within a few days of the company being registered with Companies House (incorporated).")
