  ...
  "Self Assessment Expenses view" must {
    ...
    "contain correct content" in {
      ...
      doc.text() must include(
        "You put all of your allowable expenses for the tax year on your Self-Assessment tax return. " +
          "Do not send proof of your expenses with your tax return. Keep them so that you can show them to HMRC, if asked.")

      doc.text() must include(
        "You can choose to use ‘Simplified expenses’ to work out some of your expenses using flat " +
      ...
      doc.text() must include(
        "Simplified expenses cannot be used by limited companies or business partnerships involving a " +
          "limited company.")

      doc.text() must include(
        "If you do not use simplified expenses and if your business income is £85,000 or more, you need " +
          "to enter each expense.")

      doc.text() must include(
        "If your business income is below £85,000 for the full year, you can still list your expenses, " +
      ...
      doc.text() must include(
        "You can claim either the actual costs, or a flat rate for business mileage. Once you have chosen " +
          "one of these methods for a particular vehicle, you must continue to use it for that vehicle.")

      doc.text() must include(
        "If you decide to use the actual costs, you will need to know how many miles the vehicle has done " +
          "over the year. Then work out what percentage was for business. You can also claim a capital allowance for the purchase " +
          "of the vehicle.")

      doc.text() must include(
        "The flat rate covers the whole cost of buying, running and maintaining the vehicle – so you cannot " +
    ...
    "contain the 'What expenses can I include in my Self Assessment tax return - video transcript' link" in {
      ...
      assertLinkById(
        ...
        expectedGAEvent =
          "link - click:Expenses:Expenses if you are self employed - video transcript"
      )
    }
    ...
    "contain the 'Claiming motoring expenses if you’re self-employed - video transcript' link" in {
      ...
      assertLinkById(
        ...
        expectedGAEvent =
          "link - click:Expenses:Calculating motoring expenses - video transcript"
      )
    }
