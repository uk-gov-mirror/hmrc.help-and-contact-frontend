...
trait ViewSpecBase extends SpecBase {
  ...
  def assertContainsLabel(doc: Document,
    ...
    assert(label.text() == expectedText,
  ...
  def assertElementHasClass(doc: Document,
    ...
    assert(doc.getElementById(id).hasClass(expectedClass),
  ...
  def assertContainsRadioButton(doc: Document,
    ...
    assert(radio.attr("name") == name,
           s"\n\nElement $id does not have name $name")
    assert(radio.attr("value") == value,
           s"\n\nElement $id does not have value $value")
    isChecked match {
      case true =>
        assert(radio.attr("checked") == "checked",
               s"\n\nElement $id is not checked")
      case _ =>
        assert(!radio.hasAttr("checked") && radio.attr("checked") != "checked",
  ...
  def assertLinkById(doc: Document,
    ...
    assert(link.text() == expectedText,
           s"\n\n Link $linkId does not have text $expectedText")
    assert(link.attr("href") == expectedUrl,
    ...
    assert(link.attr("data-journey-click") == expectedGAEvent,
